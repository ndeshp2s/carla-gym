...
class Spawner(object):
    ...
    def spawn_pedestrian(self, spawn_points, ev_trans = None):
        ...
        while len(self.pedestrian_list) <  self.num_of_ped:
            ...
            ped = self.world.try_spawn_actor(ped_bp, carla.Transform(carla.Location(37, -1.8, 0.5)))
    ...
    def check_pedestrian_distance(self, ev_trans = None):
        ...
        for p in self.pedestrian_list:
            ...
            if ped_loc_relative[0] > carla_config.ped_max_dist or ped_loc_relative[0] < carla_config.ped_min_dist or abs(ped_loc_relative[1]) > 10:
                if p["controller"] is not None:
                    controller = self.world.get_actor(p["controller"])
                    if controller is not None:
                        controller.stop()
    ...
    def get_goal(self, goal_points, start):
        for goal in goal_points:
            goal_wp = self.world.get_map().get_waypoint(goal.location, project_to_road = True, lane_type = carla.LaneType.Sidewalk)
            start_wp = self.world.get_map().get_waypoint(start.location, project_to_road = True, lane_type = carla.LaneType.Sidewalk)
...
def main():
    ...
    # Spawn ego vehicle
    sp = carla.Transform(carla.Location(x = 40.0, y = -0.5, z = 0.1), carla.Rotation(yaw = 180))
    ...
    try:
        while True:
            ...
            for p in spawner.pedestrian_list:
                ...
                if abs(ped_loc_relative[0]) <= 3.5 and abs(ped_loc_relative[1]) <= 1.2:
                    print('collision')

                #if is_within_distance_ahead(ped.get_transform(), veh.get_transform(), 3.5):
                #    print("collision")
